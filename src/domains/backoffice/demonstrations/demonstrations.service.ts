...
export class DemonstrationsService {
  ...
  async create(data: CreateDemonstrationDto): Promise<Demonstration> {
    return this.demonstrationRepository.save(this.demonstrationRepository.create({
      name: data.name.trim(),
      email: data.email.trim().toLowerCase(),
      phone: data.phone?.trim() || null,
      scheduledAt: new Date(data.scheduledAt),
      productInterest: data.productInterest?.trim() || null,
  ...
  async list(page = 1, amount = 10, search?: string): Promise<PaginatedResult<Demonstration>> {
    ...
    const query = this.demonstrationRepository.qb('demonstration')
    ...
    const [data, total] = await Promise.all([
      query.skip((page - 1) * amount).take(amount).getMany(),
      query.getCount(),
    ]);
  ...
  async update(id: string, data: UpdateDemonstrationDto): Promise<Demonstration> {
    ...
    return this.demonstrationRepository.update(id, {
      ...
      ...(data.productInterest !== undefined && { productInterest: data.productInterest?.trim() || null }),
