import { Body, Controller, Delete, Get, Param, ParseIntPipe, ParseUUIDPipe, Post, Put, Query, UseInterceptors } from '@nestjs/common';
...
export class DemonstrationsController {
  ...
  @UseAuth('admin')
  @UseInterceptors(TransactionInterceptor)
  async update(@Param('id', ParseUUIDPipe) id: string, @Body() data: UpdateDemonstrationDto): Promise<DemonstrationDto> {
  ...
  private toDto(data: unknown): DemonstrationDto {
    return plainToInstance(DemonstrationDto, data, { excludeExtraneousValues: true });
  }
}
